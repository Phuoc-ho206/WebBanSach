...
│   ├── fbauthcontroller.php        # Facebook OAuth
...
- Hỗ trợ login với Facebook OAuth
- Session management
...
Xem file `DATABASE_SETUP.md` để setup database.

### 2. SMTP Configuration (để gửi email OTP)

Mở file `auth/controller/forgetpasswordcontroller.php`, tìm method `sendOTPEmail()`:
...
Mở file `auth/controller/fbauthcontroller.php`:

```php
private static $appId = 'YOUR_FACEBOOK_APP_ID';
private static $appSecret = 'YOUR_FACEBOOK_APP_SECRET';
private static $redirectUri = 'http://localhost/WebBanSach/auth/pages/login.php';
```
...
### Test Register Flow
1. Truy cập `/WebBanSach/auth/pages/register.php`
2. Điền thông tin đăng ký
3. Kiểm tra database có user mới không

### Test Login Flow
1. Truy cập `/WebBanSach/auth/pages/login.php`
2. Đăng nhập với tài khoản vừa tạo
3. Kiểm tra session có user không

### Test Profile Flow
1. Đăng nhập
2. Truy cập `/WebBanSach/auth/pages/profile.php`
3. Cập nhật thông tin
4. Đổi mật khẩu

### Test Forget Password Flow
1. Truy cập `/WebBanSach/auth/pages/Forgetpassword/index.php`
2. Nhập email
3. Kiểm tra email nhận OTP (hoặc check error_log)
4. Nhập OTP
5. Đặt mật khẩu mới
...
5. (Optional) Cấu hình Google/Facebook OAuth
